...
Tinytest.addAsync('Client - observeMetrics - login', function(test, done) {
...
Tinytest.addAsync('Client - observeMetrics - observe data fetch data', function(test, done) {
...
Tinytest.addAsync('Client - observeMetrics - observe data caching for realtime queries', function(
  test,
  done
) {
    ...
    Tracker.autorun(function(c) {
      ...
      if (!timeOut) {
        ...
        timeOut = setTimeout(function() {
          test.equal(payloadList, [{ data: data, ready: true }, { data: data, ready: true }]);
          c.stop();
          handle2.stop();
          done();
        }, 100);
    ...
    function _startAgain() {
      ...
      Tracker.autorun(function(c) {
        ...
        if (!timeOut) {
          ...
          timeOut = setTimeout(function() {
            test.equal(payloadList, [{ data: data, ready: true }]);
            c.stop();
            handle2.stop();
            done();
          }, 100);
...
Tinytest.addAsync('Client - observeMetrics - ready state', function(test, done) {
  var args = { range: 60 * 60 * 1000, realtime: true, appId: 'appId' };
  var handle = KadiraData.observeMetrics('browser-metrics', args);
  Tracker.autorun(function(c) {
    if (handle.ready()) {
...
Tinytest.addAsync('Client - observeMetrics - on Error', function(test, done) {
  var args = { range: 60 * 60 * 1000, realtime: true, appId: 'appId' };
...
Tinytest.addAsync('Client - observeMetrics - when subscription stopped by the server', function(
  test,
  done
) {
...
Tinytest.addAsync('Client - observeMetrics - when subscription stopped by the client', function(
  test,
  done
) {
...
Tinytest.addAsync('Client - observeMetrics - fetch a clone of data', function(test, done) {
  var args = { range: 60 * 60 * 1000, realtime: true, appId: 'appId' };
  var handle = KadiraData.observeMetrics('browser-metrics', args);
  Tracker.autorun(function(c) {
    var data = handle.fetch();
...
Tinytest.addAsync('Client - fetchTraces - fetch traces successfully', function(test, done) {
...
Tinytest.addAsync('Client - fetchTraces - fetch traces with time related data - caching', function(
  test,
  done
) {
...
Tinytest.addAsync('Client - fetchTraces - fetch traces from the server errored', function(
  test,
  done
) {
