Tinytest.addAsync('Server - fetchTraces - no definition', function(test, done) {
  var client = GetClient();
  client.call(
    'kadiraData.fetchTraces',
    'non-existing-data-key',
    { realtime: true, appId: 'appId' },
    onResult
  );
...
Tinytest.addAsync('Server - fetchTraces - fetch trace list', function(test, done) {
  ...
  var client = GetClient();
  client.call('kadiraData.fetchTraces', dataKey, args, onResult);
...
Tinytest.addAsync('Server - fetchTraces - fetch single trace', function(test, done) {
  var dataKey = Random.id();
  var args = {
    query: { _id: 'aa' },
    appId: 'appId'
  ...
  var client = GetClient();
  client.call('kadiraData.fetchTraces', dataKey, _.clone(args), onResult);
...
Tinytest.addAsync('Server - fetchTraces - apply filters', function(test, done) {
  var dataKey = Random.id();
  var args = {
    range: 60 * 60 * 1000,
    time: new Date(),
