...
Tinytest.add('saveGlobalQueryParams when triggersEnter', function(test) {
  ...
  // check changes data
  var globalQueryParamsStore = _.pairs(UrlStateManager._globalQueryParamsStore);
...
Tinytest.add('saveSubSection when triggersEnter', function(test) {
  ...
  var currentSubSectionStore = _.pairs(UrlStateManager._currentSubSectionStore);

  var returnAppId = currentSubSectionStore[0][0];
  var returnSubSection = currentSubSectionStore[0][1][context.params.section];

  test.equal(context.params.appId, returnAppId);
  test.equal(context.params.subSection, returnSubSection);

  //_subSectionStateStore
  var subSectionStateStore = _.pairs(UrlStateManager._subSectionStateStore);
...
Tinytest.add('get appUrl by pathTo with section, noSubSection and has subSectionInStore', function(
  test
) {
  clearBeforeEachTest();
  ...
  var expectedAppUrl =
    ...
    '/' +
    context.params.subSection;

  test.equal(expectedAppUrl, appUrl);
});

Tinytest.add(
  "get appUrl by pathTo with section, noSubSection and doesn't have subSectionInStore",
  function(test) {
    clearBeforeEachTest();

    var appUrl = UrlStateManager.pathTo(context.params.appId, context.params.section);
    var expectedAppUrl =
      '/apps/' + context.params.appId + '/' + context.params.section + '/overview';

    test.equal(expectedAppUrl, appUrl);
  }
);

Tinytest.add('get appUrl by pathTo with section, subSection and states in the store', function(
  test
) {
  clearBeforeEachTest();
...
Tinytest.add(
  'get appUrl by pathTo with section, subSection and global query params in the store',
  function(test) {
    clearBeforeEachTest();
    ...
    var appUrl = UrlStateManager.pathTo(context.params.appId, 'errors', 'overview');
    var expectedAppUrl =
      '/apps/' + context.params.appId + '/errors/overview?res=1min&date=42342424';
