KadiraData.defineMetrics(
  ...
  function(args) {
    ...
    function calculateResTime(avgValue, sampleValue) {
      groupDef.sum = {
        $sum: KadiraDataHelpers.safeMultiply('$value.' + avgValue, '$value.' + sampleValue)
      };
      groupDef.samples = { $sum: '$value.' + sampleValue };
      projectDef.sortedValue = KadiraDataHelpers.divideWithZero('$sum', '$samples', true);
    }
...
KadiraData.defineMetrics(
  ...
  function(args) {
    ...
    ['wait', 'db', 'http', 'email', 'compute', 'async'].forEach(function(metric) {
      ...
      projectDef[metric] = KadiraDataHelpers.divideWithZero('$' + metric, '$count', true);
    });

    return pipes;
  },
  [
    KadiraDataFilters.roundTo(['wait', 'db', 'http', 'email', 'compute', 'async'], 2),
    KadiraDataFilters.addZeros(['wait', 'db', 'http', 'email', 'compute', 'async'])
  ]
);
...
KadiraData.defineMetrics(
  ...
  function(args) {
    ...
    projectDef.total = KadiraDataHelpers.divideWithZero('$total', '$count', true);
