...
KadiraErrorFilters.filterErrorsByStatus = function() {
  return function(data, args) {
    ...
    if (status) {
      var newData = KadiraErrorFilters._includeByStatus(data, filteredErrorsMeta, status);
      return newData;
    }
    ...
    if (showIgnored) {
      ...
      var filteredIgnored = KadiraErrorFilters._excludeByStatus(
        data,
        filteredErrorsMeta,
        'ignored'
      );
...
KadiraErrorFilters._getAllErrors = function getAllErrors(data, filteredErrorsMetaMap) {
  var newData = [];
...
KadiraErrorFilters._includeByStatus = function includeByStatus(
  data,
  filteredErrorsMetaMap,
  status
) {
  var newData = [];
  data.forEach(function(d) {
...
KadiraErrorFilters._excludeByStatus = function excludeByStatus(
  data,
  filteredErrorsMetaMap,
  status
) {
...
KadiraErrorFilters._canShowIgnored = function canShowIgnored(args) {
  var status = args.status;
...
KadiraErrorFilters._setStatusForError = function setStatusForError(found, error) {
  if (found) {
