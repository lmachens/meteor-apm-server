...
setDefinition('meteor-pub-breakdown', async function(dl, args) {
  ...
  switch (args.sortField) {
    ...
    case 'updateRatio':
      ...
      groupDef.allDocumentChanges = {
        $sum: { $add: allDocumentChangesFields }
      };

      projectDef.sortedValue = {
        $multiply: [divide('$allDocumentChanges', '$initiallyAddedDocuments', true), 100]
      };
  }
  function calculateObserverRatio() {
    ...
    projectDef.sortedValue = {
      $multiply: [divide('$cached', '$totalObservers'), 100]
    };
  }
