...
function timeFrame(date, range) {
  ...
  if (range <= _1hourRange) {
    return moment(minus).format('MMM Do YYYY HH:mm') + ' - ' + moment(plus).format('HH:mm');
  } else if (range > _1hourRange && range < _7dayRange) {
    return moment(minus).format('MMM Do YYYY HH:mm') + ' - ' + moment(plus).format('Do HH:mm');
  } else if (range >= _7dayRange) {
    return moment(minus).format('MMM Do YYYY') + ' - ' + moment(plus).format('MMM Do');
  }
}
