var component = FlowComponents.define('profiler.flameviewer', function(params) {
  ...
  this.onRendered(function() {
    ...
    this.autorun(function() {
      var cpuProfile = params.cpuProfileFn();
      if (!params.currentPathFn || !cpuProfile) {
        return;
      }
...
component.prototype._generatePath = function(pathId) {
  ...
  var maxDepth = 0;
  var generateFunctionNode = _.template(
    $('#flamebar-node')
      .text()
      .trim()
  );
  ...
  var htmlString = '<svg width="' + this.width + '" height="' + height + '">' + svgHtml + '</svg>';
...
var baseColors = ['#c1e3ab', '#b6e7bc', '#c6ecd4', '#b9e8d4', '#b3e0a3'];
