MapReduce = async function (db, sourceColl, outCollection, map, reduce, options) {
  ...
  await db.collection(sourceColl).find(query).forEach(function(d) {
  ...
  var bulk = await db.collection(outCollection).initializeOrderedBulkOp();
