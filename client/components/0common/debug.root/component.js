...
component.prototype.setActivities = function() {
  ...
  if (activityTime) {
    ...
    activities = this.store.aggregateActivities(currentSession, startTime, endTime, options);
  } else {
    activities = this.store.aggregateLastActivities(currentSession, 1, activitySort);
  }
...
component.prototype.showTraceAt = function(e) {
  ...
  if (AVAILABLE_TRACES[e.type]) {
    ...
    this.store.getTrace(browserId, clientId, type, id, function(err, trace) {
      if (trace) {
