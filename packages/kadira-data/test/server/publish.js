Tinytest.addAsync('Server - observeMetrics - no definition', function(test, done) {
  var client = GetClient();
  client.subscribe(
    'kadiraData.observeMetrics',
    'some-id',
    'non-existing-data-key',
    { realtime: true, appId: 'appId', range: 60 * 60 * 1000 },
    { onStop: afterClosed }
  );
...
Tinytest.addAsync('Server - observeMetrics - send data without realtime', function(test, done) {
  ...
  var transportColl = new Mongo.Collection(KadiraData._transportCollection, options);
  var sub = client.subscribe('kadiraData.observeMetrics', transctionId, dataKey, args, {
    onStop: onStop,
    onReady: onReady
  });
...
Tinytest.addAsync('Server - observeMetrics - send data with realtime 1min', function(test, done) {
  var dataKey = Random.id();
  var transctionId = Random.id();
  var args = {
    range: 60 * 60 * 1000,
  ...
  var client = GetClient();
  var transportColl = new Mongo.Collection(KadiraData._transportCollection, { connection: client });
  client.subscribe('kadiraData.observeMetrics', transctionId, dataKey, args, { onReady: onReady });
...
Tinytest.addAsync('Server - observeMetrics - send data with querying for host', function(
  test,
  done
) {
  var dataKey = Random.id();
  ...
  var client = GetClient();
  client.subscribe('kadiraData.observeMetrics', transctionId, dataKey, args);
});

Tinytest.addAsync('Server - observeMetrics - apply filters', function(test, done) {
  var dataKey = Random.id();
  var transctionId = Random.id();
  var args = {
    range: 60 * 60 * 1000,
