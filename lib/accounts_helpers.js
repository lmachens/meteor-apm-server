...
AccountsHelpers.getUserEmail = function _getUserEmail(user) {
  if (AccountsHelpers.isMeteorLoginConnected(user)) {
    var primaryEmail = _.find(user.services && user.services['meteor-developer'].emails, function(
      email
    ) {
    ...
    var email = primaryEmail.address || user.services['meteor-developer'].emails[0].address;
    return email;
